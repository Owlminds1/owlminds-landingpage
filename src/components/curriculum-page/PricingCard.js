...
const fetchPricingData = async (countryCode) => {
  const response = await axios.get(
    `https://dev-api.owlminds.com/api/public/index.php?action=get_pricing&country=${countryCode}`
  );
  const apiData = response.data.data[countryCode] || [
    { plan_type: "Basic", price: "399.00", per_class_price: "50.00" },
    { plan_type: "Standard", price: "1199.00", per_class_price: "37.00" },
    { plan_type: "Premium", price: "1899.00", per_class_price: "29.00" },
  ];
  return apiData;
};
...
export default function PricingCard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const apiPrices = await fetchPricingData(country);
      const updatedPricingData = pricingCardData.map((card, index) => ({
        ...card,
        correctPrice: `$${apiPrices[index].price}` || card.correctPrice,
        inCorrectPrice:
          `$${(parseFloat(apiPrices[index].price) * 1.2).toFixed(2)}` ||
          card.inCorrectPrice,
      }));
    ...
    <>
      <div className="max-w-3xl md:max-w-4xl lg:max-w-7xl mx-auto mb-6 sm:mb-10 px-4 sm:px-2 text-black">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-y-8 sm:gap-y-0">
          {pricingData.map((priceCard, index) => (
            <div
              key={priceCard.id}
            ...
            >
              ...
              >
                <Image
                  ...
                  alt={"price-card-1"}
                  className={`
                  ${priceCard.active ? "w-full h-[45rem]" : "w-full h-full"}`}
                />
                <div
                ...
                >
                  {priceCard.type}
                  <span className="font-normal sm:text-lg  text-sm ml-2">
                    {priceCard.classType}
                  </span>
                  <span className="font-normal-700 text-sm text-[#F04438] bg-white float-right sm:p-2 p-1 rounded-[30px] mt-1">
                    {priceCard.ClassLeft}
                  </span>
                  <div
                    className={`flex items-end mt-5 ${
                      priceCard.active ? "" : "sm:py-6 py-1"
                    }`}
                  >
                    <p
                            ...
                            <g clip-path="url(#clip0_1199_9671)">
                              <g clip-path="url(#clip1_1199_9671)">
                                <path
                                  fill-rule="evenodd"
                                  clip-rule="evenodd"
                                  d="M0.847656 12.1647C0.847656 8.98207 2.11194 5.92983 4.36238 3.67939C6.61281 1.42895 9.66506 0.164673 12.8477 0.164673C16.0303 0.164673 19.0825 1.42895 21.3329 3.67939C23.5834 5.92983 24.8477 8.98207 24.8477 12.1647C24.8477 15.3473 23.5834 18.3995 21.3329 20.65C19.0825 22.9004 16.0303 24.1647 12.8477 24.1647C9.66506 24.1647 6.61281 22.9004 4.36238 20.65C2.11194 18.3995 0.847656 15.3473 0.847656 12.1647ZM12.1629 17.3007L19.0717 8.66387L17.8237 7.66547L11.9325 15.0271L7.75966 11.5503L6.73566 12.7791L12.1629 17.3007Z"
                                  fill="#7F00FF"
                                />
                              </g>
                            </g>
                            <defs>
                              <clipPath id="clip0_1199_9671">
                                <rect
                                  ...
                                  height="24.6424"
                                  fill="white"
                                  transform="translate(0.495605 0.166138)"
                                />
                              </clipPath>
                              <clipPath id="clip1_1199_9671">
                                <rect
                                  ...
                                  height="24"
                                  fill="white"
                                  transform="translate(0.847656 0.164673)"
                                />
                              </clipPath>
                            </defs>
                          </svg>
                            ...
                            <defs>
                              <clipPath id="clip0_1199_9707">
                                <rect
                                  ...
                                  height="24.6424"
                                  fill="white"
                                  transform="translate(0.651367 0.0904541)"
                                />
                              </clipPath>
                              <clipPath id="clip1_1199_9707">
                                <rect
                                  ...
                                  height="24"
                                  fill="white"
                                  transform="translate(1.00342 0.0889893)"
                                />
                              </clipPath>
                            </defs>
                          </svg>
      ...
      <div className="text-center p-4 mt-4">
        <p className="font-bold text-purple-700 text-[24px]">
          100% Risk-Free – Unused Classes? Money-Back Guarantee!
        </p>
        <p className="text-black text-[15px]">
          We believe in delivering value. If your child has unused classes,
          we’ll refund the amount for those sessions—no questions asked!
        </p>
      </div>
    </>
  );
